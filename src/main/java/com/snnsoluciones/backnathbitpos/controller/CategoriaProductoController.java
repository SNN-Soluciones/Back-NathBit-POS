...
public class CategoriaProductoController {
  ...
  public ResponseEntity<ApiResponse<List<CategoriaProductoResponse>>> listarPorEmpresa(
    ...
    List<CategoriaProducto> categorias = categoriaService.listarPorEmpresa(empresaId, sucursalId,
        busqueda);
  ...
  public ResponseEntity<ApiResponse<CategoriaProductoResponse>> crear(
    ...
    try {
      ...
      if (categoriaService.existePorNombreYEmpresa(request.getNombre(), request.getSucursalId(),
          request.getEmpresaId())) {
        return ResponseEntity.badRequest()
            .body(ApiResponse.error("Ya existe una categoría con ese nombre"));
      }

      Sucursal sucursal = sucursalService.finById(request.getSucursalId()).orElse(null);
  ...
  public ResponseEntity<ApiResponse<CategoriaProductoResponse>> actualizar(
    ...
    try {
      ...
      categoriaExistente.setSucursal(sucursalService.finById(request.getSucursalId()).orElse(null));
  ...
  @PreAuthorize("hasAnyRole('ROOT', 'SOPORTE', 'SUPER_ADMIN', 'ADMIN')")
  public ResponseEntity<ApiResponse<CategoriaProductoResponse>> toggleActivo(
      @PathVariable Long id) {
  ...
  private CategoriaProductoResponse convertirAResponse(CategoriaProducto categoria) {
    return CategoriaProductoResponse.builder()
        ...
        .sucursalId(categoria.getSucursal() != null ? categoria.getSucursal().getId() : null)
