...
public class TiqueteInternoPdfMapperService {
    ...
    public Map<String, Object> mapearTiqueteInternoAParametros(String numeroInterno) {
        ...
        parametros.put("empresa_nombre", empresa.getNombreComercial() != null ?
        ...
        parametros.put("empresa_telefono", empresa.getTelefono());
        parametros.put("empresa_direccion", obtenerDireccionEmpresa(empresa));

        if(sucursal != null && !sucursal.getLogoSucursalPath().isEmpty()) {
            parametros.put("logo_empresa", new ByteArrayInputStream(storageService.downloadFileAsBytes(sucursal.getLogoSucursalPath())));
        ...
        parametros.put("terminal", "Terminal 01"); // FacturaInterna no tiene terminal, usar valor por defecto
        ...
        // Vuelto (si aplica)
        if (facturaInterna.getVuelto() != null && facturaInterna.getVuelto().compareTo(BigDecimal.ZERO) > 0) {
