...
public class ReporteVentasServiceImpl implements ReporteVentasService {
    ...
    public ReporteVentasResponse obtenerDatosReporte(ReporteVentasRequest request) {
        ...
        String sql = """
        -- ======================================
        -- FACTURAS ELECTRÓNICAS (las existentes)
        -- ======================================
        SELECT
          f.clave,
          f.consecutivo,
        ...
        -- ======================================
        -- 🆕 FACTURAS INTERNAS (nuevas)
        -- ======================================
        SELECT
          NULL AS clave,
          fi.numero AS consecutivo,
          'TIQUETE_INTERNO' AS tipoDocumento,
          -- 🔥 SOLUCIÓN: Formatear fecha como ISO con offset
          TO_CHAR(fi.fecha AT TIME ZONE 'America/Costa_Rica', 'YYYY-MM-DD"T"HH24:MI:SS.US"-06:00"') AS fechaEmision,
        
          ae.codigo AS actividadEconomicaCodigo,
          ae.descripcion AS actividadEconomicaDescripcion,
        
          COALESCE(c.razon_social, fi.nombre_cliente, 'CLIENTE GENERICO') AS clienteNombre,
          COALESCE(c.numero_identificacion, '000000000') AS clienteIdentificacion,
          COALESCE(c.tipo_identificacion, 'OTRO') AS clienteTipoIdentificacion,
        
          -- SERVICIOS (facturas internas no separan, todo va como gravado)
          0 AS totalServiciosGravados,
          0 AS totalServiciosExentos,
          0 AS totalServiciosExonerados,
          0 AS totalServiciosNoSujetos,
        
          -- MERCANCÍAS (todo el subtotal va aquí)
          COALESCE(fi.subtotal, 0) AS totalMercanciasGravadas,
          0 AS totalMercanciasExentas,
          0 AS totalMercanciasExoneradas,
          0 AS totalMercanciasNoSujetos,
        
          -- TOTALES
          COALESCE(fi.subtotal - fi.descuento, 0) AS totalVentaNeta,
          0 AS totalImpuesto,
          COALESCE(fi.descuento, 0) AS totalDescuentos,
          0 AS montoTotalExonerado,
          COALESCE(fi.total, 0) AS totalComprobante,
          
          0 AS totalOtrosCargos,
          
          COALESCE(fi.subtotal, 0) AS subtotalGravado,
          0 AS subtotalExento,
          0 AS subtotalExonerado,
          0 AS subtotalNoSujetos,
        
          'CRC' AS moneda,
          1 AS tipoCambio
        
        FROM factura_interna fi
        INNER JOIN sucursales s ON fi.sucursal_id = s.id
        LEFT JOIN empresa_actividades ea ON ea.empresa_id = s.empresa_id AND ea.es_principal = true
        LEFT JOIN actividades_economicas ae ON ea.actividad_id = ae.id
        LEFT JOIN clientes c ON fi.cliente_id = c.id
        ...
        List<ReporteVentasLineaDTO> lineas = jdbcTemplate.query(
            sql,
            rowMapper,
